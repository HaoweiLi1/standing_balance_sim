...
class DataPlotter:
    ...
    def plot_dashboard(self, show=True, save=False):
        ...
        ax6 = fig.add_subplot(gs[3, :])  # Hip data (new)
        ...
        # Add hip data plot if available
        if all(name in self.data for name in ["hip_position", "hip_velocity", "hip_torque"]):
            hip_pos_data = self.data["hip_position"]
            hip_vel_data = self.data["hip_velocity"]
            hip_torque_data = self.data["hip_torque"]
            
            ax6.plot(hip_pos_data[:, 0], hip_pos_data[:, 1], label="Hip Position", color='b')
            ax6.plot(hip_vel_data[:, 0], hip_vel_data[:, 1] / 10, label="Hip Velocity / 10", color='g')
            ax6.plot(hip_torque_data[:, 0], hip_torque_data[:, 1], label="Hip Torque", color='r')
            ax6.set_xlabel("Time [s]")
            ax6.set_ylabel("Value")
            ax6.set_title("Hip Joint Data")
    ...
    # def plot_hip_data(self, show=True, save=False):
    #     """
    #     Plot hip joint position, velocity, and torque.
    ...
    #     """
    #     # Check if required datasets exist
    #     required = ["hip_position", "hip_velocity", "hip_torque"]
    #     for name in required:
    #         if name not in self.data:
    #             print(f"Error: Required dataset '{name}' not found.")
    #             return
    ...
    #     # Get data
    #     pos_data = self.data["hip_position"]
    #     vel_data = self.data["hip_velocity"]
    #     torque_data = self.data["hip_torque"]
        
    #     # Plot position
    #     ax1.plot(pos_data[:, 0], pos_data[:, 1], label="Hip Position", color='b')
    #     ax1.set_ylabel("Angle [deg]")
    #     ax1.set_title("Hip Joint Position")
    #     ax1.grid(True, linestyle='--', alpha=0.7)
        
    #     # Plot velocity
    #     ax2.plot(vel_data[:, 0], vel_data[:, 1], label="Hip Velocity", color='g')
    #     ax2.set_ylabel("Angular Velocity [deg/s]")
    #     ax2.set_title("Hip Joint Velocity")
    #     ax2.grid(True, linestyle='--', alpha=0.7)
        
    #     # Plot torque
    #     ax3.plot(torque_data[:, 0], torque_data[:, 1], label="Hip Torque", color='r')
    #     ax3.set_xlabel("Time [s]")
    #     ax3.set_ylabel("Torque [Nm]")
    #     ax3.set_title("Hip Joint Torque")
    #     ax3.grid(True, linestyle='--', alpha=0.7)
    ...
    #         plot_dir = os.path.join(self.data_dir, 'plots')
    #         os.makedirs(plot_dir, exist_ok=True)
    #         plt.savefig(os.path.join(plot_dir, "hip_data.png"), dpi=300, bbox_inches='tight')
    ...
    def generate_all_plots(self, show=False, save=True):
        ...
        # Add this line to generate hip data plot
        self.plot_hip_data(show=show, save=save)
