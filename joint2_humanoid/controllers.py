...
class HipPDController(Controller):
    ...
    def compute_control(self, state, target):
        ...
        if delta_time > 0:  # Avoid division by zero
            self.current_rtd = (limited_torque - self.prev_torque) / delta_time
        else:
            self.current_rtd = 0.0
...
def create_hip_controller(controller_type, model, data, params):
    ...
    Returns:
        An instance of a Controller subclass for hip
    """
    if controller_type == "PD":
        return HipPDController(
            model=model,
            data=data,
            max_torque=params.get('max_torque', 150),
