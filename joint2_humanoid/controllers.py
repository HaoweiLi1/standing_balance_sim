...
class HipController(Controller):
    """Base class for hip joint controllers."""
    
    def __init__(self, model, data, mrtd=None):
        super().__init__(model, data)
        self.mrtd = mrtd  # Maximum rate of torque development for hip
        self.prev_torque = 0.0
        self.current_rtd = 0.0
        self.current_rtd_limit = 0.0
    ...
    def apply_mrtd_limits(self, desired_torque, timestep):
        ...
        # For hip, we use a symmetric RTD limit
        max_delta = self.mrtd * timestep
        rtd_limit = self.mrtd
        
        if abs(delta_torque) > max_delta:
            delta_torque = np.sign(delta_torque) * max_delta
            self.current_rtd = np.sign(delta_torque) * self.mrtd
        else:
            self.current_rtd = raw_rtd
...
class HipPDController(HipController):
    """PD controller for hip joint."""
    
    def __init__(self, model, data, kp, kd, mrtd=None):
        super().__init__(model, data, mrtd)
        self.kp = kp
        self.kd = kd
        self.prev_error = 0
...
class HipNoneController(HipController):
    """Controller that always returns zero torque (disabled hip)."""
...
def create_hip_controller(controller_type, model, data, params):
    """
    Factory function to create a hip controller instance based on type.
    ...
    Returns:
        An instance of a HipController subclass
    """
    if controller_type == "PD":
